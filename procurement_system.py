...
# KEY DECISION: "expanded" so sidebar is always rendered in DOM.
# We hide it visually on login with width:0, NOT display:none
# (display:none removes it from DOM permanently in Streamlit SPA)
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
...
def log_action(oid, action, prev, nxt, by, detail):
    ...
    n = pd.DataFrame([{"id":str(len(D["activity_log"])+1),"order_id":oid,
...
for k, v in {"logged_in":False,"username":"","role":"","user_name":"",
              "page":"Dashboard","co_filter":"All","login_error":""}.items():
    if k not in st.session_state: st.session_state[k] = v
if "D" not in st.session_state: st.session_state.D = load_data()

# ─────────────────────────────────────────────────────────────────────────────
# GLOBAL STYLES  ← injected once on every render
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
/* ═══ UNIVERSAL INPUT FIX ═══════════════════════════════════════════════
   These selectors cover every known Streamlit version + Chromium quirks.
   We use BOTH 'color' AND '-webkit-text-fill-color' because Chrome ignores
   'color' when the browser's autofill or theme overrides kick in.       */
input, textarea,
...
def topbar(title, sub=""):
    sub_h = f'<div style="font-size:12px;color:#64748b;margin-top:3px;">{sub}</div>' if sub else ""
    st.markdown(f'''
    <div style="background:#fff;border-bottom:1px solid #e2e8f0;padding:14px 28px;
         display:flex;justify-content:space-between;align-items:center;
         box-shadow:0 1px 3px rgba(0,0,0,.04);">
      <div>
        <div style="font-size:18px;font-weight:800;color:#0f172a;">{title}</div>{sub_h}
      </div>
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="display:flex;align-items:center;gap:6px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:20px;padding:5px 14px;font-size:11.5px;font-weight:600;color:#15803d;">
          <div style="width:7px;height:7px;border-radius:50%;background:#22c55e;"></div>Auto-saved</div>
        <div style="font-size:12px;color:#64748b;">📅 {datetime.now().strftime("%d %b %Y")}</div>
      </div>
    </div>
...
# ─────────────────────────────────────────────────────────────────────────────
def login_page():
    # ── Sidebar: collapse to width 0, keep in DOM ────────────────────────
    # IMPORTANT: we use width/min-width/overflow, NOT display:none.
    # display:none kills the sidebar element permanently in Streamlit SPA.
    st.markdown("""
    <style>
    .stApp { background: linear-gradient(135deg,#0c1426 0%,#1e3a5f 50%,#0c1426 100%) !important; }

    /* Collapse sidebar width to zero — keeps it in DOM, just invisible */
    ...
    /* Hide the collapse arrow button too */
    ...
    with col:
        # ── Static HTML branding (no widgets inside) ─────────────────────
        st.markdown("""
        <div style="background:#ffffff;border:1px solid #e2e8f0;
             border-radius:16px 16px 0 0;padding:36px 32px 20px;text-align:center;">
          <div style="width:72px;height:72px;border-radius:16px;margin:0 auto 16px;
               background:linear-gradient(135deg,#1e3a5f,#1d4ed8);
               display:flex;align-items:center;justify-content:center;
               font-size:36px;box-shadow:0 4px 18px rgba(29,78,216,.4);">🏭</div>
        ...
        <div style="background:#ffffff;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;
          ...
          <div style="font-size:14px;font-weight:700;color:#0f172a;">Sign in to your account</div>
          <div style="font-size:12px;color:#64748b;margin-top:4px;">Enter your credentials below</div>
        </div>
        """, unsafe_allow_html=True)

        # ── White card wrapper around the form ───────────────────────────
        # This ensures the Streamlit widget area sits on white,
        # so our input CSS (#FFFFFF bg, #111827 text) works correctly.
        st.markdown('<div style="background:#ffffff;padding:4px 32px 0;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">', unsafe_allow_html=True)

        # ── LOGIN FORM ── st.form = atomic submit + Enter-key support ────
        with st.form("login_form", clear_on_submit=False):
            uname = st.text_input("Username", placeholder="e.g. Admin",          key="li_u")
            sp(4)
            pword = st.text_input("Password", placeholder="Enter your password",
                                   type="password", key="li_p")
            sp(10)
            # Inline CSS to make the submit button red
            st.markdown("""
            <style>
        ...
        <div style="background:#ffffff;border:1px solid #e2e8f0;border-top:none;
             border-radius:0 0 16px 16px;padding:14px 32px 26px;
             box-shadow:0 24px 60px rgba(0,0,0,.5);">
          <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;
               padding:10px 14px;font-size:11.5px;color:#64748b;line-height:2;">
            <strong style="color:#374151;">Demo credentials:</strong><br>
            <code>Admin</code>&nbsp;/&nbsp;admin@123
            &nbsp;·&nbsp;
            <code>Manager</code>&nbsp;/&nbsp;mgr@123<br>
            <code>Staff</code>&nbsp;/&nbsp;Ops@Secure#1
            &nbsp;·&nbsp;
            <code>Viewer</code>&nbsp;/&nbsp;view123
          </div>
...
# ─────────────────────────────────────────────────────────────────────────────
def render_sidebar():
    # Restore sidebar to full size now that we're logged in
    st.markdown("""
    <style>
    ...
    with st.sidebar:
        ...
        <div style="padding:20px 16px 16px;border-bottom:1px solid #1e293b;">
          <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div>
              <div style="font-size:13px;font-weight:800;color:#f8fafc;">Supply Chain</div>
              <div style="font-size:9px;color:#475569;letter-spacing:1px;text-transform:uppercase;">Tracking System</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        sp(8)
        st.markdown('<div style="padding:0 16px;font-size:9px;font-weight:700;letter-spacing:1.2px;color:#334155;text-transform:uppercase;margin-bottom:4px;">NAVIGATION</div>', unsafe_allow_html=True)
        ...
        <div style="padding:0 12px;margin-bottom:10px;">
          <div style="background:#1e293b;border-radius:10px;padding:10px 12px;
            ...
            <div style="overflow:hidden;">
              ...
              <span style="font-size:10px;font-weight:700;background:{rb};color:{rc};
                   padding:1px 7px;border-radius:4px;">{role}</span>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
def page_dashboard():
    ...
    kpis=[("Total Orders",len(fil),"#1d4ed8"),
    ...
    for col,(lbl,val,color) in zip(st.columns(6,gap="small"),kpis):
        with col:
            st.markdown(f'<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:18px 14px 14px;box-shadow:0 1px 4px rgba(0,0,0,.05);position:relative;overflow:hidden;"><div style="position:absolute;top:0;left:0;right:0;height:3px;background:{color};border-radius:12px 12px 0 0;"></div><div style="font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#64748b;margin-bottom:8px;">{lbl}</div><div style="font-size:26px;font-weight:800;color:{color};line-height:1;">{val}</div></div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True); sp(20)
    ...
    st.markdown('</div>',unsafe_allow_html=True); sp(18)
    ...
    if len(fil)==0:
        ...
        for i,(_,r) in enumerate(fil.iterrows()):
            ...
            tbody+=f'<tr style="background:{bg}"><td style="{TD}font-weight:700;color:#1d4ed8;font-size:11.5px;white-space:nowrap;">{r["order_id"]}</td><td style="{TD}">{cobadge(r["company"])}</td><td style="{TD}color:#64748b;font-size:11.5px;">{r["po_number"]}</td><td style="{TD}font-size:12px;">{r["govt_department"]}</td><td style="{TD}font-size:12px;">{desc}</td><td style="{TD}text-align:center;font-weight:600;">{r["quantity"]}</td><td style="{TD}font-weight:700;white-space:nowrap;">₹{float(r["total_value"]):,.0f}</td><td style="{TD}">{pbadge(r["priority"])}</td><td style="{TD}">{sbadge(r["current_status"])}</td><td style="{TD}color:#94a3b8;font-size:11px;white-space:nowrap;">{r["last_updated"]}</td></tr>'
    label="All Orders" if cf=="All" else f"{cf} Orders"
    st.markdown(f'<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.05);"><div style="display:flex;justify-content:space-between;align-items:center;padding:12px 18px;border-bottom:1px solid #f1f5f9;background:#fafbfc;"><div style="font-size:14px;font-weight:700;color:#0f172a;">📦 {label}</div><div style="font-size:11px;font-weight:600;color:#64748b;background:#f1f5f9;padding:3px 12px;border-radius:20px;">{len(fil)} orders</div></div><div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;"><thead><tr><th style="{TH}">Order ID</th><th style="{TH}">Company</th><th style="{TH}">PO#</th><th style="{TH}">Department</th><th style="{TH}">Description</th><th style="{TH}">Qty</th><th style="{TH}">Value</th><th style="{TH}">Priority</th><th style="{TH}">Status</th><th style="{TH}">Updated</th></tr></thead><tbody>{tbody}</tbody></table></div></div>',unsafe_allow_html=True)
...
def page_new_order():
    ...
    D=st.session_state.D; P="padding:0 28px;"
    st.markdown(f'<div style="{P}">',unsafe_allow_html=True)
...
def page_update_order():
    ...
    with tab2:
        with st.form("disp_form"):
            c1,c2=st.columns(2)
            with c1: d_date=st.date_input("Dispatch Date",key="d_date"); d_cour=st.text_input("Courier *",placeholder="e.g. BlueDart"); d_veh=st.text_input("Vehicle No.",placeholder="e.g. DL01AB1234")
    ...
    with tab3:
        with st.form("del_form"):
            c1,c2=st.columns(2)
            with c1: de_stat=st.selectbox("Delivery Status",["","Delivered","Partial","Failed","Rescheduled"]); de_date=st.date_input("Delivery Date",key="de_date"); de_recv=st.text_input("Receiver Name")
...
def page_order_details():
    ...
    for i,step in enumerate(STATUSES):
        if i<cur_idx:   cc,lc="#1d4ed8;color:#fff","#1d4ed8"
        elif i==cur_idx: cc,lc="#dbeafe;border:2px solid #1d4ed8;color:#1d4ed8","#1d4ed8"
        ...
        if i<len(STATUSES)-1: html+=f'<div style="flex:1;height:2px;background:{"#1d4ed8" if i<cur_idx else "#e2e8f0"};margin:0 3px 16px;"></div>'
    ...
    with t5:
        ...
        for _,log in logs.iterrows():
            ...
            st.markdown(f'<div style="display:flex;gap:10px;background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:11px 14px;margin-bottom:7px;"><div style="width:8px;height:8px;border-radius:50%;background:#3b82f6;flex-shrink:0;margin-top:4px;"></div><div style="flex:1;"><div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:5px;"><div style="display:flex;align-items:center;gap:5px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;background:#f1f5f9;padding:1px 7px;border-radius:5px;">{log["action_type"]}</span>{pb}{nb}</div><span style="font-size:10.5px;color:#94a3b8;">{log["performed_at"]}</span></div><div style="font-size:12px;color:#374151;margin-top:4px;">{log["details"]} — <b>{log["performed_by"]}</b></div></div></div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True); footer()
...
def page_activity_log():
    ...
    P="padding:0 28px;"
    st.markdown(f'<div style="{P}">',unsafe_allow_html=True)
    st.markdown(f'<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.05);"><div style="display:flex;justify-content:space-between;align-items:center;padding:12px 18px;border-bottom:1px solid #f1f5f9;background:#fafbfc;"><div style="font-size:14px;font-weight:700;color:#0f172a;">📋 All System Events</div><div style="font-size:11px;font-weight:600;color:#64748b;background:#f1f5f9;padding:3px 12px;border-radius:20px;">{len(logs)} events</div></div><div style="padding:14px;">',unsafe_allow_html=True)
    for _,log in logs.iterrows():
        prev=log.get("previous_status",""); ns_l=log.get("new_status","")
        pb=sbadge(prev) if prev and prev not in ["—","nan",""] else '<span style="color:#94a3b8">—</span>'
        nb="&nbsp;→&nbsp;"+sbadge(ns_l) if ns_l and ns_l not in ["nan",""] else ""
        st.markdown(f'<div style="display:flex;gap:10px;background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:11px 14px;margin-bottom:7px;"><div style="width:8px;height:8px;border-radius:50%;background:#3b82f6;flex-shrink:0;margin-top:4px;"></div><div style="flex:1;"><div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:5px;"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;background:#f1f5f9;padding:1px 7px;border-radius:5px;">{log["action_type"]}</span><span style="font-size:11.5px;font-weight:700;color:#1d4ed8;">{log["order_id"]}</span>{pb}{nb}</div><span style="font-size:10.5px;color:#94a3b8;white-space:nowrap;">{log["performed_at"]}</span></div><div style="font-size:12px;color:#374151;margin-top:4px;">{log["details"]} — <b>{log["performed_by"]}</b></div></div></div>',unsafe_allow_html=True)
...
def page_reports():
    ...
    def prog_card(col,title,items,key_bg,key_fg,label_fn,val_fn):
        with col:
            st.markdown(f'<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.05);margin-bottom:18px;"><div style="padding:12px 18px;border-bottom:1px solid #f1f5f9;background:#fafbfc;"><div style="font-size:13.5px;font-weight:700;color:#0f172a;">{title}</div></div><div style="padding:16px;">',unsafe_allow_html=True)
            for it in items:
                lbl=label_fn(it); val=val_fn(it,df)
                pct=int(len(df[df["company"]==it])/max(len(df),1)*100) if it in COMPANIES else int(len(df[df["current_status"]==it])/max(len(df),1)*100)
                fg=key_fg.get(it,"#64748b"); bg=key_bg.get(it,"#f1f5f9")
                st.markdown(f'<div style="margin-bottom:14px;"><div style="display:flex;justify-content:space-between;margin-bottom:5px;"><span style="display:inline-block;padding:2px 9px;border-radius:6px;font-size:11px;font-weight:700;background:{bg};color:{fg};">{lbl}</span><span style="font-size:12px;font-weight:700;color:{fg};">{val}</span></div><div style="background:#f1f5f9;border-radius:4px;height:5px;"><div style="width:{pct}%;background:{fg};border-radius:4px;height:5px;"></div></div></div>',unsafe_allow_html=True)
            st.markdown('</div></div>',unsafe_allow_html=True)
    prog_card(c1,"🏢 Orders by Company",COMPANIES,CB,CF,
              lambda c:c,lambda c,d:f"{len(d[d['company']==c])} orders · ₹{d[d['company']==c]['total_value'].sum()/1e5:.1f}L")
    prog_card(c2,"📊 Orders by Status",STATUSES,SB,SF,
              lambda s:f"{SE.get(s,'')} {s}",lambda s,d:str(len(d[d["current_status"]==s])))
    ...
    st.markdown(f'<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.05);margin-bottom:18px;"><div style="padding:12px 18px;border-bottom:1px solid #f1f5f9;background:#fafbfc;"><div style="font-size:13.5px;font-weight:700;color:#0f172a;">💵 Financial Summary</div></div><div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;"><thead><tr><th style="{TH}">Order ID</th><th style="{TH}">Company</th><th style="{TH}">Department</th><th style="{TH}">Value</th><th style="{TH}">Status</th><th style="{TH}">Paid</th><th style="{TH}">Outstanding</th></tr></thead><tbody>{rows}</tbody></table></div><div style="padding:10px 18px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:13px;font-weight:700;"><span style="color:#64748b;">Total Outstanding</span><span style="color:#92400e;">₹{outstanding:,.0f}</span></div></div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True); footer()
...
def page_admin():
    ...
    D=st.session_state.D; P="padding:0 28px;"
    st.markdown(f'<div style="{P}">',unsafe_allow_html=True)
    st.info("✅ **Auto-Save** is on. All data persists automatically. Use export buttons below.")
    ...
    stat_cells="".join([f'<div style="background:#f8fafc;border:1px solid #e2e8f0;border-top:3px solid #1d4ed8;border-radius:8px;padding:10px;text-align:center;"><div style="font-size:9.5px;font-weight:700;color:#64748b;text-transform:uppercase;">{t}</div><div style="font-size:20px;font-weight:800;color:#1d4ed8;margin-top:5px;">{len(D[t])}</div><div style="font-size:9.5px;color:#94a3b8;">rows</div></div>' for t in tables])
