...
input[type="email"], input[type="search"], input[type="date"], textarea,
...
.stDateInput input, .stDateInput > div > div > input,
...
.stButton > button[kind="primary"] {
    background: #1d4ed8 !important; border: 1.5px solid #1d4ed8 !important;
...
.stTabs [data-baseweb="tab-list"] {
    background: transparent !important; border-bottom: 2px solid #e2e8f0 !important;
...
.stTabs [data-baseweb="tab"] {
    ...
    padding: 10px 20px !important; margin-bottom: -2px !important;
}
...
def topbar(title, sub=""):
    ...
    <div style="background:#fff;border-bottom:1px solid #e2e8f0;padding:14px 28px;
      ...
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div style="font-size:12px;color:#64748b;font-weight:500;">📅 {datetime.now().strftime("%d %b %Y")}</div>
      </div>
    </div>
...
def section_label(text):
    st.markdown(
        ...
        f'color:#94a3b8;border-bottom:1px solid #f1f5f9;padding-bottom:8px;margin:14px 0 12px;">{text}</div>',
        unsafe_allow_html=True)
...
def login_page():
    ...
    /* FIX: Labels must be DARK on white card background, NOT white */
    .stTextInput label,
    ...
    with col:
        # Branding card (HTML only — no widgets)
        st.markdown("""
        ...
        # Login form — st.form ensures Enter key works + zero timing issues
        with st.form(key="login_form", clear_on_submit=False):
            uname = st.text_input("Username", placeholder="e.g. Admin", key="fi_user")
            pword = st.text_input("Password", placeholder="Enter your password",
                                   type="password", key="fi_pass")
            sp(8)
            submitted = st.form_submit_button("🔐  Sign In",
                                               use_container_width=True, type="primary")

        # Handle OUTSIDE the form (Streamlit requirement)
        if submitted:
            u = uname.strip()
            p = pword
...
def page_dashboard():
    ...
    for col, lbl, key in [
        (fc1,"🏢 All Companies","All"),(fc2,"Robokart","Robokart"),
...
def page_new_order():
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;
      ...
      </div><div style="padding:24px;">""", unsafe_allow_html=True)
    ...
    with st.form("new_order_form", clear_on_submit=True):
        ...
        remarks   = st.text_area("Remarks / Special Instructions", placeholder="Any additional notes…", height=70)
        sp(6)
    ...
    if submitted:
        ...
        if errs:
            ...
            st.error("⚠️  PO Number already exists.")
        else:
            ts  = now_ist()
            ...
            save("orders")
            log_action(oid,"ORDER_CREATED","—","Pending",st.session_state.user_name,
...
def page_update_order():
    ...
    with tab1:
        ...
        with st.form("proc_form"):
            ...
            if sv:
                if not updated_by: st.error("Please enter your name in the field above the tabs.")
                else:
                    ts=now_ist()
                    D["procurement"]=pd.concat([D["procurement"],pd.DataFrame([{
                        ...
                        "notes":p_notes,"updated_by":updated_by,"updated_at":ts}])],ignore_index=True)
                    save("procurement")
                    ns="Procured" if p_stat=="Completed" else order["current_status"]
                    prev=order["current_status"]; set_status(oid,ns)
                    log_action(oid,"STATUS_CHANGE",prev,ns,updated_by,f"Procurement: {p_stat}")
            ...
            if sv:
                if not updated_by: st.error("Please enter your name in the field above the tabs.")
                    ...
                    D["dispatch"]=pd.concat([D["dispatch"],pd.DataFrame([{
                        ...
                        "updated_by":updated_by,"updated_at":ts}])],ignore_index=True)
                    save("dispatch")
                    prev=order["current_status"]; set_status(oid,"Dispatched")
                    log_action(oid,"STATUS_CHANGE",prev,"Dispatched",updated_by,f"Dispatched via {d_cour}")
    ...
    with tab3:
        ...
        with st.form("del_form"):
            ...
            del_f=st.file_uploader("Delivery files",accept_multiple_files=True,key="dlf",label_visibility="collapsed")
            st.markdown("**Challan Documents**")
            ch_f =st.file_uploader("Challan files",accept_multiple_files=True,key="chf",label_visibility="collapsed")
    ...
    with tab4:
        ...
        with st.form("inv_form"):
            ...
            inv_f=st.file_uploader("Invoice files",accept_multiple_files=True,key="ivf",label_visibility="collapsed")
...
def page_order_details():
    ...
    for i,step in enumerate(STATUSES):
        ...
        html+=(f'<div style="display:flex;flex-direction:column;align-items:center;flex-shrink:0;">'
               ...
               f'display:flex;align-items:center;justify-content:center;font-size:14px;">{SE.get(step,"")}</div>'
              ...
              <div style="flex:1;">
                <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px;">
                  <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                    <span style="font-size:11.5px;font-weight:700;background:#f1f5f9;padding:2px 8px;border-radius:5px;">{log['action_type']}</span>
                    {pb}{nb}
                  </div>
                  <span style="font-size:11px;color:#94a3b8;">{log['performed_at']}</span>
                </div>
                <div style="font-size:12.5px;color:#374151;margin-top:5px;">{log['details']} — <b>{log['performed_by']}</b></div>
              </div>
            </div>""", unsafe_allow_html=True)
...
def page_activity_log():
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;
      ...
      </div><div style="padding:16px;">""", unsafe_allow_html=True)
    for _,log in logs.iterrows():
        prev=log.get("previous_status",""); ns_l=log.get("new_status","")
        pb=(sbadge(prev) if prev and prev not in ["—","nan",""] else '<span style="color:#94a3b8">—</span>')
        nb=("&nbsp;→&nbsp;"+sbadge(ns_l) if ns_l and ns_l not in ["nan",""] else "")
...
def page_reports():
    ...
    def prog_card(col,title,items,key_bg,key_fg,label_fn,val_fn):
        with col:
            ...
            for it in items:
                ...
                pct=int(len(df[df["company"]==it])/max(len(df),1)*100) if it in COMPANIES \
    ...
    for i,(_,r) in enumerate(df.iterrows()):
        ...
        rows+=(f'<tr style="background:{bg}"><td style="{TD}font-size:11.5px;color:#64748b;">{r["order_id"]}</td>'
               f'<td style="{TD}">{cobadge(r["company"])}</td><td style="{TD}">{r["govt_department"]}</td>'
               f'<td style="{TD}font-weight:700;">₹{tv:,.0f}</td><td style="{TD}">{sbadge(r["current_status"])}</td>'
               f'<td style="{TD}">{paid}</td><td style="{TD}">{out}</td></tr>')
    st.markdown(f"""
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;
...
def page_admin():
    ...
    st.markdown(f'<div style="{P}">', unsafe_allow_html=True)
    st.info("✅ **Auto-Save** — All changes persist instantly.  📥 **Export** — Download any table as CSV below.")

    tables=["orders","procurement","dispatch","delivery","invoices","activity_log"]
    st.markdown("""<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;
         box-shadow:0 1px 4px rgba(0,0,0,0.05);margin-bottom:20px;">
      <div style="padding:14px 20px;border-bottom:1px solid #f1f5f9;background:#fafbfc;">
    ...
    sp(8)
    all_csv="\n\n".join([f"=== {t.upper()} ===\n"+D[t].to_csv(index=False) for t in tables]).encode("utf-8")
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;
      ...
      <div style="padding:18px;display:grid;grid-template-columns:repeat(6,1fr);gap:12px;">{stat_cells}</div>
    </div>""", unsafe_allow_html=True)
    ...
    with st.expander("⚠️ Danger Zone — Reset All Data to Sample"):
        st.warning("⚠️ This will permanently delete all entered data and restore the 4 sample orders.")
        if st.button("🗑  Reset to Sample Data",type="primary",key="reset_btn"):
            seed=make_seed()
            for k,d in seed.items(): d.to_csv(FILES[k],index=False)
            st.session_state.D=load_data()
            st.success("✅ Data reset to sample. Reloading…"); st.rerun()

    st.markdown('</div>', unsafe_allow_html=True)
    footer()

# ══════════════════════════════════════════════════════════════════════════════
# 16. ROUTER  ← THE FIX: main() must be called at the bottom!
# ══════════════════════════════════════════════════════════════════════════════
def main():
...
# ══════════════════════════════════════════════════════════════════════════════
# ✅ FIX #1 — main() call was MISSING from the original code → blank page
# ══════════════════════════════════════════════════════════════════════════════
main()
